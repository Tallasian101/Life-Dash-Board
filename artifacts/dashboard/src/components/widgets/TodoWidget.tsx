...
export function TodoWidget() {
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (newTodo.trim()) {
      createMutation.mutate({ data: { text: newTodo.trim() } });
    }
  };
            ...
            <div className="space-y-2">
              {todos.map((todo) => (
                <div
                  key={todo.id}
                  className={cn(
                    "group/item flex items-center gap-3 p-2.5 rounded-md border border-transparent hover:bg-white/[0.02] hover:border-white/5 transition-all duration-200",
                ...
                >
                  <button
                    ...
                    className={cn(
                      "flex-shrink-0 h-5 w-5 rounded flex items-center justify-center border transition-colors",
                      todo.completed 
